...
Currently, this project is not available in a package repository. As such, users will need to manually download the files (e.g. =git clone https://github.com/krisbalintona/denote-zettel-interface.git=) alongside its dependencies. Currently, these are the only third-party dependencies:
...
Users can either evaluate something like this:
